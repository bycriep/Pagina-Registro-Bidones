document.addEventListener("DOMContentLoaded", async () => {
    ...
    const cargarVentasPorRepartidor = async (repartidor = "", fecha = "", quincena = 0) => {
        try {
            const res = await fetch(`/api/ventas/historial?repartidor=${repartidor}&fecha=${fecha}&quincena=${quincena}`, {
                headers: { "x-auth-token": token }
            });
    ...
    const filtrarVentas = async () => {
        ...
        const quincena = document.getElementById("filtroFecha").value;

        let quincenaValor = 0;
        if (quincena === "quincena1") quincenaValor = 1;
        else if (quincena === "quincena2") quincenaValor = 2;

        cargarVentasPorRepartidor(repartidor, fecha, quincenaValor);
    };

    ["filtroFecha", "filtroRepartidor"].forEach(id =>
    ...
    const exportar = async (tipo) => {
        try {
            ...
            const query = new URLSearchParams({ fecha, repartidor, quincena: quincenaValor }).toString();
            const res = await fetch(`/api/ventas/exportar/${tipo}?${query}`, {
                headers: { "x-auth-token": token }
            });
